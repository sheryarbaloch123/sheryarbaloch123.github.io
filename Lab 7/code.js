...
function enableUnitConversion() {
  hideAllModes();
  document.getElementById("unitConversionMode").classList.remove("hidden");
}

function enableCurrencyConversion() {
  hideAllModes();
  document.getElementById("currencyConversionMode").classList.remove("hidden");
}
...
document.addEventListener("DOMContentLoaded", function () {
  // Setup event listeners for menu toggling
  document.getElementById("menuBtn").addEventListener("click", function () {
    document.getElementById("submenu").classList.toggle("hidden");
  });

  // Setup event listeners for conversion functionalities
  document
    .getElementById("convertUnitButton")
    .addEventListener("click", convertUnit);
  document
    .getElementById("convertCurrencyButton")
...
function convertUnit() {
  ...
  switch (unitType) {
    case "length":
      output = `${value} meters is ${(value * 3.28084).toFixed(2)} feet.`;
      break;
    case "weight":
      output = `${value} kilograms is ${(value * 2.20462).toFixed(2)} pounds.`;
      break;
    case "temperature":
      output = `${value}°C is ${(value * 1.8 + 32).toFixed(2)}°F.`;
      break;
    default:
      output = "Please select a valid conversion type.";
  }

  document.getElementById("unitOutput").innerText = output;
}
...
function convertCurrency() {
  ...
  // Example conversion rates; adjust these based on real-time data if available
  if (fromCurrency === "USD" && toCurrency === "EUR") {
    result = `${amount} USD is ${(amount * 0.93).toFixed(2)} EUR.`;
  } else if (fromCurrency === "EUR" && toCurrency === "USD") {
    result = `${amount} EUR is ${(amount * 1.07).toFixed(2)} USD.`;
  } else {
    result = "Conversion not available for the selected currencies.";
  }

  document.getElementById("currencyOutput").innerText = result;
}
